...
general_inflation = [0.00 for _ in range(num_periods)]
price_ag_list = [1.0]
price_ener_list = [1.0]
price_hous_list = [1.0]
...
def initialize_households_and_sales_from_struct(households_struct, firms, params, num_sectors=6,
    ...
    for h in households_struct:
        income_init = random.uniform(0.25, 0.75)
        savings_init = random.uniform(0.25, 0.75)
...
# === NOUVEAU BLOC : Production dynamique et demande d'emploi fluctuante ===
    ...
    # Fluctuation réaliste de l'organisation du travail
        ...
        for firm in firm_list:
            ...
            firm["BrownCapital"].append((updated_brown_cap) * 0.99)
            firm["GreenCapital"].append((firm["GreenCapital"][t - 1]) * 0.99 + green_investment)
...
#            RBU_amount = 1  # Revenu de base universel fixe par ménage et période
            income_tax_threshold = 3  # Seuil de revenu à partir duquel la taxe s'applique
        ...
        for household in household_list:
            ...
            # === Étape t=0 ===
            if t == 0:
                household["ConsAg"].append(0.002)
                household["ConsEner"].append(0.002)
                household["ConsHous"].append(0.002)
                household["ConsTrans"].append(0.002)
...
#            print(f"[t={t}] Conso totale: {total_conso:.2f} | Revenus firmes: {total_revenue:.2f}")
            ...
            # Pondérations
            w_needs = random.uniform(0.1, 0.9)
            w_bandwagon = random.uniform(0.1, 0.9)
            w_inertia = random.uniform(0.1, 0.9)

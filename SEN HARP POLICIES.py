...
#            if t < len(firm["BrownCapital"]):
        ...
        for household in household_list:
            ...
            RBU_amount = 1 * np.prod([(1 + general_inflation[i]) for i in range(t + 1)])
...
#        total_consumption = sum(h["Consumption"][t] for h in household_list)
    ...
    # 2. Investissement total (brown + green) des firmes
        ...
        if PostGrowthActive[t] == 1:
            # Construire l’éligibilité par nom de secteur (à partir des ids 1..6)
            _pg = (globals().get("pg_cons", {}) or {})
            _sigma = (_pg.get("sigma_target", {}) or {})
            _name_by_id = {
                1: "Agriculture", 2: "Energy", 3: "Housing",
                4: "Transport", 5: "Industry", 6: "Technology"
            }
            eligible = {
                _name_by_id[sid]: (1.0 if float(_sigma.get(sid, 0.0)) > 0.0 else 0.0)
                for sid in _name_by_id
            }
            ...
            share_pg = g_shared_share(
                ...
                scenario_name,
                PostGrowthActive,
                {"ramp_periods": 3, "target_share": 0.6}  # 3 périodes de montée
            )
            ...
            # Scénarios non post-growth : conso inchangée, pas de socialisation
            sector_after_social = sector_revenue_market.copy()
            G_socialized_t = 0.0
        ...
        total_public_spending_t = total_public_spending
